...
`scripts/generate-image.mjs` and `scripts/generate-image-edit.mjs` are the
working CLI versions (`npm run gen`, `npm run gen:edit`), both copies of the
portable `generate-image-openai` skill.
